...
const NewRecordModal = ({ show, onHide, onSave, selectedDate }: NewRecordModalProps) => {
  ...
  const loadLookupData = async () => {
    try {
      ...
      console.error(err);
      await message("Erro ao carregar opções: " + err, {
        title: "Erro",
        kind: "error",
      });
    }
  ...
  const handleSubmit = async () => {
    ...
    if (
      ...
      await message("Por favor, preencha todos os campos obrigatórios.", {
        title: "Atenção",
        kind: "warning",
      });
      return;
    ...
    <Modal show={show} onHide={onHide} centered size="lg">
      ...
      <Modal.Body className="p-4">
        <p className="text-muted mb-4">
          Preencha os dados do atleta para o relatório diário.
        </p>
        ...
        <Form>
            ...
            <div className="col-md-6">
              <Form.Group className="mb-3">
                ...
                <Form.Select
                  ...
                  onChange={(e) =>
                    setFormData({ ...formData, complaintId: e.target.value })
                  }
                  className="py-2"
                >
                  <option value="">Selecione...</option>
          ...
          <Form.Group className="mb-4">
            <Form.Label className="fw-semibold small text-muted text-uppercase d-block">
              Status do Atleta
            </Form.Label>
            <div className="d-flex gap-3">
              {lookupData?.status.map((s) => (
                <div
                  key={s.id}
                  onClick={() =>
                    setFormData({ ...formData, statusId: s.id.toString() })
                  }
